...
4. Run `10_workspace_email_capture.sql` to enable user-chosen, capture-only `alias@wrxs.cc` routes. Before any route is enabled, configure Postmark inbound mail and Netlify DNS, then deploy `wrxs-email-capture` with `WRXS_POSTMARK_INBOUND_TOKEN`. The legacy `crm@iacy.com` receiver remains separate and unchanged.
