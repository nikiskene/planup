...
export default function ServiceSettings() {
  ...
  useEffect(() => {
    let cancelled = false;
    setLoading(true); setError(''); setSubscriptions([]); setRuntime(null);
    async function load() {
      ...
      try {
        ...
        if (canManage) {
          const subscriptionResult = await client.from('wrxs_subscriptions').select('status,billing_interval,access_until,workspace_id').eq('workspace_id', activeWorkspaceId);
          if (subscriptionResult.error) throw subscriptionResult.error;
          if (!cancelled) setSubscriptions(subscriptionResult.data);
        }
